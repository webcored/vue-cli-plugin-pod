...
class Generate extends Common {
  async exec(api: apiTypeMock, _options: null, args: string[]): Promise<void> {
    ...
    if (this.fileType === 'component') {
      ...
      const templateFile: FileOptions | undefined = typeConfig.files.find(
        (file) => file.isTemplateFile
      );
      ...
      if (!templateFile) {
        this.log(
          'warning',
          'Template file missing, loading file content will fail.'
        );
      } else {
        ...
        if (typeConfig.singleFile) {
          ...
          generatableFiles = typeConfig.files.filter(
            (file) => file.isTemplateFile || !file.tagname
          );
        } else {
  ...
  private singFileTemplateContent(tagfiles: FileOptions[]) {
    ...
    tagfiles.forEach((file) => {
      ...
      templateFileContent += `<${file.tagname}${
  ...
  private multiFileTemplateContent(tagfiles: FileOptions[]) {
    ...
    const templateFileObject: FileOptions | undefined = tagfiles.find(
      (file) => file.isTemplateFile
    );
    ...
    tagfiles.forEach((file) => {
      ...
      templateFileContent += `<${file.tagname}${
        attributes ? ` ${attributes}` : ''
      }>${tagInnerContent ? `${tagInnerContent}` : ''}</${file.tagname}> \n\n`;
    });
